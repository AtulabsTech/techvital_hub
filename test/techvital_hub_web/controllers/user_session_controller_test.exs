defmodule TechvitalHubWeb.UserSessionControllerTest do
  ...
  describe "POST /login" do
    test "logs the user in", %{conn: conn, user: user} do
      token =
        extract_user_token(fn url ->
          TechvitalHub.Accounts.deliver_user_confirmation_instructions(user, url)
        end)

      {:ok, confirmed_user} = TechvitalHub.Accounts.confirm_user(token)
    ...
    test "logs the user in with remember me", %{conn: conn, user: user} do
      ...
      assert conn.resp_cookies["_techvital_hub_web_user_remember_me"]
      assert redirected_to(conn) == ~p"/dashboard"
    end
