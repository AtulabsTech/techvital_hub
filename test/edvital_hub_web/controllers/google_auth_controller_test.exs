defmodule EdvitalHubWeb.GoogleAuthControllerTest do
  ...
  alias EdvitalHubWeb.GoogleAuthController
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, EdvitalHubWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "GET /auth/google/callback callback" do
    test "redirects to login path when token is invalid", %{conn: conn} do
      conn =
        conn
        |> bypass_through(EdvitalHubWeb.Router, [:browser])
